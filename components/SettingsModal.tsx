...
import { X, User, CreditCard, Settings as SettingsIcon, Save, Key, Shield, Bell } from 'lucide-react';
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-5 duration-300">
             ...
             <div className="p-5 rounded-2xl bg-gradient-to-br from-primary-900/20 to-surface border border-primary-500/20 flex justify-between items-center">
                <div>
                   ...
                   <h4 className="text-2xl font-bold text-white mb-1">专业版 Pro</h4>
             ...
             <div className="space-y-2">
                ...
                <div className="bg-black/20 rounded-xl border border-white/5 p-8 text-center text-gray-500 text-xs">
                   暂无充值记录
                </div>
             </div>
          </div>
        ...
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-5 duration-300">
            ...
            <p className="text-sm text-gray-400 leading-relaxed">
              您可以绑定自己的 API Key 以享受独立配额。
            </p>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">供应商</label>
      ...
      <div className="relative bg-[#18181b] border border-white/10 rounded-3xl w-full max-w-4xl h-[600px] shadow-2xl flex overflow-hidden animate-in zoom-in-95 fade-in duration-300">
         <div className="w-64 bg-black/20 border-r border-white/5 p-6 flex flex-col">
            ...
            <div className="space-y-1">
               <button onClick={() => setActiveTab('profile')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'profile' ? 'bg-primary-600/10 text-primary-400' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                  <User size={18} /> 个人设置
               </button>
               <button onClick={() => setActiveTab('billing')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'billing' ? 'bg-primary-600/10 text-primary-400' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                  <CreditCard size={18} /> 账单与订阅
               </button>
               <button onClick={() => setActiveTab('apikey')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${activeTab === 'apikey' ? 'bg-primary-600/10 text-primary-400' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                  <SettingsIcon size={18} /> API Key 管理
               </button>
            </div>
         </div>
         <div className="flex-1 flex flex-col relative">
            <button onClick={onClose} className="absolute top-6 right-6 w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors z-10">
               <X size={18} />
            </button>
